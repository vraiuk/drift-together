...
**Сеть:** два пути подключения — прямое IP (порт 7777, работает сразу) и Unity
Relay с 6-значным кодом. ⚠️ Для Relay проект нужно один раз привязать к Unity
Cloud (Project Settings → Services в Unity Editor, вход в аккаунт → Link
project; затем включить Relay в dashboard.unity.com). Пока проект не привязан,
кнопка Relay честно сообщает об этом; прямое IP-подключение работает без
каких-либо сервисов.
